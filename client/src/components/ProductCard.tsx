...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <Link to={`/product/${product.id}`} className="block group">
            <div className="bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg shadow-black/10 dark:shadow-black/40 hover:shadow-xl transition-shadow duration-200 overflow-hidden h-full flex flex-col">
                {/* Product Image - Fixed aspect square */}
                <div className="aspect-square overflow-hidden bg-white dark:bg-slate-800 shrink-0">
                ...
                <div className="p-2 sm:p-3 md:p-4 flex-1 flex flex-col min-h-0">
                    ...
                    <div className="h-8 sm:h-10 md:h-12 mb-1 sm:mb-2">
                        <h3 className="font-medium text-gray-900 dark:text-white text-xs sm:text-sm line-clamp-2 group-hover:text-amber-300 transition-colors overflow-hidden h-full truncate">
                            {product.name}
                        </h3>
                    </div>
                    ...
                    <div className="mt-auto flex items-center gap-1 sm:gap-2">
                        ...
                        {product.discountPrice &&
                            product.discountPrice !== product.price && (
                                <span className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 line-through">
